...
test.describe('Date Validation & Logic', () => {
	...
	test('should handle November to January year wrap-around', async ({ page }) => {
		// Assigned: 11/27, Due: 1/5 (Valid: Jan is next year)
		await page.locator('#assigned').fill('11/27');
		await page.locator('#due').fill('1/5');

		// Check inputs are NOT flagged
		await expect(page.locator('label[for="assigned"]')).not.toHaveClass(/text-red-400/);
		await expect(page.locator('label[for="due"]')).not.toHaveClass(/text-red-400/);
	});
