...
import { STATUS_TYPE, COMM_ASSIGNMENT_TYPES, LEVEL_TYPE, DATE_FIELDS } from '../constants';
...
describe('Communication Constants', () => {
	...
	describe('COMM_ASSIGNMENT_TYPES', () => {
		it('should have correct number of assignment types', () => {
			expect(COMM_ASSIGNMENT_TYPES.length).toBe(5);
		});

		it('should have all required properties for each assignment type', () => {
			COMM_ASSIGNMENT_TYPES.forEach(assignment => {
		...
		it('should have G9 assignments marked correctly', () => {
			const g9Assignments = COMM_ASSIGNMENT_TYPES.filter(a => a.isG9);
		...
		it('should have CLIL assignments marked correctly', () => {
			const clilAssignments = COMM_ASSIGNMENT_TYPES.filter(a => a.isCLIL);
