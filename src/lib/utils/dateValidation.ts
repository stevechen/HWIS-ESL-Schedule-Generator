 ...
 * Handles year wrap-around: if one date is in December (12) and the other in January (1),
...
export function compareDates(d1: string, d2: string): number {
	...
	// Special case: December vs January wrap-around
	if (m1 === 12 && m2 === 1) return -1; // d1 (Dec) is earlier than d2 (Jan next year)
	if (m1 === 1 && m2 === 12) return 1; // d1 (Jan next year) is later than d2 (Dec)
